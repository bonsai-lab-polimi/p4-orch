...
def send_packets_to_host(srcMAC, srcIP, dstMAC, dstIP, payload_size, iface, name, min_throughput, max_throughput,
    ...
    for i in range(1, total_packets + 1):
        try:
            ...
            if name == "h2":  # h2: Invio rapido (throughput elevato)
            ...
            elif name == "h1":  # h4: Velocità fluttuante
                throughput = min_throughput + (max_throughput - min_throughput) * (
                        1 - math.cos(math.pi * (elapsed_time % cycle_time) / cycle_time)) / 2
            elif name == "h4":  # h5: Invio alternato (rapido e poi pausa lunga)
...
def main():
    ...
    destinations = [
        ...
        {"mac": "08:00:00:00:04:44", "ip": "10.0.1.4", "name": "h4"},  # Host 4 (h4)
        {"mac": "08:00:00:00:01:11", "ip": "10.0.1.1", "name": "h1"},  # Host 5 (h5)
    ]
    ...
    min_throughput = 15_000  # 5 kbps
    max_throughput = 120_000  # 70 kbps
