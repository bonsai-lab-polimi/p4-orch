    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        for sw, params in switches.items():
            ...
            if "cpu_port" in params:
                print(f"Switch {sw}: cpu_port is set to {params['cpu_port']}")
                self.addSwitch(sw, log_file="%s/%s.log" %(log_dir, sw), cpu_port=params["cpu_port"], cls=switchClass)
            else:
                print(f"Switch {sw}: cpu_port is not defined, setting to 510")
                params["cpu_port"] = 510
                self.addSwitch(sw, log_file="%s/%s.log" %(log_dir, sw), cpu_port=params["cpu_port"], cls=switchClass)
...
class ExerciseRunner:
    ...
    def program_switch_p4runtime(self, sw_name, sw_dict):
        ...
        self.logger('Configuring switch %s with id %s using P4Runtime with file %s' % (sw_name, device_id, runtime_json))
